...
namespace WHToolkit.Database
{
    ...
    public class MsSqlHelper : IDisposable
    {
        ...
        /// SQL Server 연결 객체
        /// </summary>
        public SqlConnection Connection => _dbConnection.Value;
        ...
        public MsSqlHelper()
        {
            _dbConnection = new(() => new SqlConnection(connectionString));
        }
        ...
        public MsSqlHelper(string customConnectionString)
        {
            _dbConnection = new(() => new SqlConnection(customConnectionString));
        }
        ...
        public DataSet ExecuteDataSet(CommandType type, string query)
        {
            lock (Connection)
            {
                try
                {
                    ...
                    using (var command = CreateCommand(query, type))
                    using (var adapter = new Microsoft.Data.SqlClient.SqlDataAdapter((SqlCommand)command))
                    {
        ...
        private SqlCommand CreateCommand(string query, CommandType commandType)
        {
            ...
            foreach (var param in Parameters)
            {
                ...
                {
                    sqlParameter = command.Parameters.AddWithValue(OutPrefix + param.ParameterName, param.Value ?? DBNull.Value);
                }
                else
                {
                    sqlParameter = command.Parameters.AddWithValue(InPrefix + param.ParameterName, param.Value ?? DBNull.Value);
                }
