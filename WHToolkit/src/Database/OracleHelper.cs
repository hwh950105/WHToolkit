...
namespace WHToolkit.Database
{
    ...
    public class OracleHelper : IDisposable
    {
        ...
        /// Oracle 연결 객체
        /// </summary>
        public OracleConnection Connection => _dbConnection.Value;
        ...
        public OracleHelper()
        {
            _dbConnection = new(() => new OracleConnection(connectionString));
        }
        ...
        public OracleHelper(string customConnectionString)
        {
            _dbConnection = new(() => new OracleConnection(customConnectionString));
        }
        ...
        private OracleCommand CreateCommand(string query, CommandType commandType)
        {
            ...
            foreach (var param in Parameters)
            {
                ...
                {
                    oracleParameter = command.Parameters.Add(OutPrefix + param.ParameterName, OracleDbType.Varchar2, param.Size > 0 ? param.Size : 4000, param.Value ?? DBNull.Value, param.Direction);
                }
                else
                {
                    oracleParameter = command.Parameters.Add(InPrefix + param.ParameterName, param.Value ?? DBNull.Value);
                }

                oracleParameter.Direction = param.Direction;
                if (param.Size > 0)
                {
                    oracleParameter.Size = param.Size;
                }
            }
